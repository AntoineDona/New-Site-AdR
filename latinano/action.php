...
<title>LATINANO</title><link rel="icon" type="image/png" href="img/adr_ico.png" />
	...
	if ($_SESSION["is_representant"] == false) {
		...
		if (!$_SESSION['shotgun']) {
			if (number_place($pdo) + $_SESSION["fsize"] < 880) {
				$query=$pdo->prepare("INSERT into latinano (prenom,nom, email, heure, taille) VALUES (?,?,?,?,?)");
