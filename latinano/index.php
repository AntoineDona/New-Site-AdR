...
{
	$query = $pdo->prepare("SELECT COUNT(*) as c from latinano where email=?");
	$query->execute(array($email));
	$result = $query->fetch();
	return $result['c'];
}
...
{
	$query = $pdo->prepare("SELECT id from latinano where email=?");
	$query->execute(array($email));
	$result = $query->fetch();
	return $result['id'];
}
...
// 	header("Location: https://adr.cs-campus.fr/latinano/agathe.php");
// }
