...
<body>
	...
	{
		$sql = 'DELETE from astronano WHERE email=:email';
		$stmt = $pdo->prepare($sql);
		$stmt->bindValue(':email', $email);
		$res = $stmt->execute();
		$_SESSION['shotgun'] = false;
		header("Location: /astronano/index.php");
	}
		...
		if ($_SESSION["is_cotisant"] == false) {
			...
			if (!$_SESSION['shotgun']) {
				if (number_place($pdo) < $_SESSION['total_places']) {
					...
					header("refresh:5; url=/astronano/index.php");
				} else {
					header("refresh:5; url=/astronano/fini.php");
				}
			} else {
