...
function has__already_shotgun($email, $pdo)
{
	...
	$query = $pdo->prepare("SELECT COUNT(*) as c from astronano where email=?");
...
if (number_place($pdo) >= $_SESSION['total_places'] and !$_SESSION['shotgun']) {
	if ($_SESSION["isConnected"]) {
		header("Location: /astronano/fini.php");
	} else {
		header("Location: /astronano/connexion.php");
	}
}
...
<head>
	...
	<link rel="shortcut icon" sizes="96x96" type="image/png" href="/astronano/img/icon.png">
...
<div id="sg_link_ctnr">
		...
		if (!$_SESSION['shotgun']) {
			...
			echo '<div id="qrLink"><a  href="/astronano/QRgenerator.php" >Voir mon QR code</a></div>';
		}
		?>
	<div id="titre_sg"></div>
	<div id="counter">
	...
	if (!$_SESSION['shotgun']) {
		...
		echo "<p id='ok_msg_ctnr'> Bravo " . $_SESSION["prenom"] . " ! <br> Tu as réussi à shotgun ta place pour l'AstroNano. </p>";
	}
	if (!$_SESSION['shotgun']) {
